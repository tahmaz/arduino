...
for(int i=0;i<4;i++) servo[i].SetPosition(90);
  String input = "";
     ...
     while (Serial.available() > 0)
     {
         ...
         delay(3); // Delay for 5 ms so the next char has time to be received
     }
  //Serial.println("con1: "+input.substring(0));
  if (input.substring(0, 1) == "w")
  {
    walk(2,t*2);
    //accept command
  ...
  else  if (input.substring(0, 2) == "kl")
  {
    kickLeft(t);
    //accept command
  ...
  else  if (input.substring(0, 2) == "kr")
  {
    kickRight(t);
    //accept command
  ...
  {
    Serial.print("con1: Command not accepted from controller1.");
  }
